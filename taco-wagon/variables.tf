...
variable "company_name" {
  ...
  validation {
    ...
    error_message = "The company_name must be alphanumeric."
  }
}
...
variable "aws_region" {
  ...
  validation {
    condition     = can(regex("^(us-(east|west|central)-[0-9]{1,2})$", var.aws_region))
    error_message = "The aws_region must be a valid US region."
  }
}
